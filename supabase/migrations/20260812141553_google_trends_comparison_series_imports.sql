-- Preserve every named term in a manually exported Google Trends comparison.
-- The source file checksum identifies the export; series_key identifies the
-- stable column within that export. Neither is a search-volume measurement.
ALTER TABLE market_intelligence.trends_imports
  ADD COLUMN IF NOT EXISTS series_key text NOT NULL DEFAULT 'single';

ALTER TABLE market_intelligence.trends_imports
  DROP CONSTRAINT IF EXISTS trends_imports_file_checksum_key;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'trends_imports_series_key_check'
      AND conrelid = 'market_intelligence.trends_imports'::regclass
  ) THEN
    ALTER TABLE market_intelligence.trends_imports
      ADD CONSTRAINT trends_imports_series_key_check
      CHECK (series_key ~ '^[a-z0-9][a-z0-9:_-]{0,119}$');
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'trends_imports_file_checksum_series_key_key'
      AND conrelid = 'market_intelligence.trends_imports'::regclass
  ) THEN
    ALTER TABLE market_intelligence.trends_imports
      ADD CONSTRAINT trends_imports_file_checksum_series_key_key
      UNIQUE (file_checksum, series_key);
  END IF;
END;
$$;

CREATE INDEX IF NOT EXISTS trends_imports_file_checksum_series_key_idx
  ON market_intelligence.trends_imports (file_checksum, series_key);
...
DECLARE
  ...
  v_series_key text;
  v_metric_interpretation text;
BEGIN
  IF coalesce((SELECT auth.jwt() ->> 'role'), '') <> 'service_role' THEN
    RAISE EXCEPTION 'market_intelligence_forbidden' USING ERRCODE = '42501';
  END IF;

  v_series_key := coalesce(nullif(p_metadata ->> 'series_key', ''), 'single');
  v_metric_interpretation := coalesce(
    nullif(p_metadata ->> 'metric_interpretation', ''),
  ...
  IF length(btrim(p_query_text)) NOT BETWEEN 1 AND 500
    ...
    OR v_series_key !~ '^[a-z0-9][a-z0-9:_-]{0,119}$'
    OR v_metric_interpretation NOT IN (
      'relative_interest_index_0_100_not_search_volume',
  ...
  INSERT INTO market_intelligence.trends_imports (
    ...
    period_start, period_end, file_checksum, series_key, metadata
  ) VALUES (
    ...
    p_period_start, p_period_end, p_file_checksum, v_series_key,
    p_metadata || jsonb_build_object(
      'dataset_kind', p_dataset_kind,
      'metric_interpretation', v_metric_interpretation
    )
  )
  ON CONFLICT (file_checksum, series_key) DO UPDATE SET
...
RETURNS TABLE (
  evidence_key text, category text, metric_key text, metric_value numeric,
  metric_unit text, period_start date, period_end date, claim_summary text,
  strategic_relevance text, publisher text, source_title text, source_url text,
  published_on date, confidence numeric
)
LANGUAGE sql
STABLE
SECURITY INVOKER
...
AS $$
  WITH latest_time_imports AS (
    ...
    WHERE s.source_key = 'google_trends' AND ti.country_code = 'US'
      AND ti.metadata ->> 'dataset_kind' = 'interest_over_time'
    ORDER BY ti.normalized_query, ti.extracted_at DESC
  ), latest_time_values AS (
    SELECT DISTINCT ON (it.import_id) it.import_id, it.relative_interest AS latest_interest
    ...
    SELECT 'google-trends-time-' || ti.id::text AS evidence_key,
      'Google Trends'::text AS category, 'relative_interest_index'::text AS metric_key,
      max(it.relative_interest) AS metric_value,
      'relative interest index (0-100; not search volume)'::text AS metric_unit,
      min(it.observed_on) AS period_start, max(it.observed_on) AS period_end,
      ...
      'Google Trends'::text AS publisher, 'Manual CSV export: ' || ti.query_text AS source_title,
      'https://trends.google.com/trends/'::text AS source_url, ti.extracted_at::date AS published_on,
    ...
    SELECT * FROM (
      SELECT 'google-trends-geo-' || ti.id::text || '-' || row_number() OVER (
          PARTITION BY ti.id ORDER BY gm.relative_interest DESC, gm.region_name
        )::text AS evidence_key,
        'Google Trends'::text AS category,
        CASE WHEN ti.metadata ->> 'metric_interpretation' =
          'comparison_share_percentage_0_100_within_exact_export_not_search_volume'
          THEN 'comparison_share_percentage' ELSE 'relative_interest_index' END::text AS metric_key,
        gm.relative_interest AS metric_value,
        CASE WHEN ti.metadata ->> 'metric_interpretation' =
          'comparison_share_percentage_0_100_within_exact_export_not_search_volume'
          THEN 'comparison share percentage within exact exported term set (not search volume)'
          ELSE 'relative interest index (0-100; not search volume)' END::text AS metric_unit,
        ti.period_start, ti.period_end,
        CASE WHEN ti.metadata ->> 'metric_interpretation' =
          'comparison_share_percentage_0_100_within_exact_export_not_search_volume'
          THEN 'Google Trends manual comparison export for "' || ti.query_text || '" assigned a '
            || gm.relative_interest::text || '% comparison share in ' || gm.region_name || '.'
          ELSE 'Google Trends manual export for "' || ti.query_text || '" recorded a relative-interest index of '
            || gm.relative_interest::text || ' in ' || gm.region_name || '.' END AS claim_summary,
        CASE WHEN ti.metadata ->> 'metric_interpretation' =
          'comparison_share_percentage_0_100_within_exact_export_not_search_volume'
          THEN 'This is a within-export comparison share, not audience size, demand, or search volume.'
          ELSE 'This is a relative geographic signal in this export only; do not infer audience size or total demand.' END::text AS strategic_relevance,
    ...
    SELECT 'google-trends-related-' || ti.id::text || '-' || rt.relation_kind || '-' || rt.ranking_kind || '-' || rt.rank::text AS evidence_key,
      'Google Trends'::text AS category,
  ...
  FROM time_evidence
  UNION ALL SELECT evidence_key, category, metric_key, metric_value, metric_unit, period_start, period_end,
    claim_summary, strategic_relevance, publisher, source_title, source_url, published_on, confidence FROM geo_evidence
  UNION ALL SELECT evidence_key, category, metric_key, metric_value, metric_unit, period_start, period_end,
    claim_summary, strategic_relevance, publisher, source_title, source_url, published_on, confidence FROM related_evidence
  ORDER BY confidence DESC, evidence_key ASC LIMIT 60;
$$;
